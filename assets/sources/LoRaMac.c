...
static void OnRadioTxDone( void )
{
    printf("OnRadioTxDone func start...\n");
	...
	printf("OnRadioTxDone: IsRxWindowsEnabled is false -> McpsConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK\n");
    ...
    {
        McpsConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK;
	// printf("OnRadioTxDone: NodeAckRequested -> McpsConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK\n");
...
static void PrepareRxDoneAbort( void )
{
    printf("PrepareRxDoneAbort func start...\n");
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    printf("OnRadioRxDone func start..\n");
    ...
    switch( macHdr.Bits.MType )
    {
	    ...
	    printf("OnRadioRxDone func case FRAME_TYPE_JOIN_ACCEPT\n");
            if( IsLoRaMacNetworkJoined == true )
            {
		printf("OnRadioRxDone func case FRAME_TYPE_JOIN_ACCEPT -> ISLoRaMacNetworkJoined == True\n");
            ...
            if( micRx == mic )
            {
		printf("OnRadioRxDone func case FRAME_TYPE_JOIN_ACCEPT -> if micRx == mic start...\n");
		    ...
		    printf("OnRadioRxDone func case FRAME_TYPE_JOIN_ACCEPT -> if micRx == mic start... -> RxDelay\n");
                }
                LoRaMacParams.ReceiveDelay1 *= 1e3;
                LoRaMacParams.ReceiveDelay2 = LoRaMacParams.ReceiveDelay1 + 1e3;
		printf("OnRadioRxDone func case FRAME_TYPE_JOIN_ACCEPT -> if micRx == mic end...\n");
		...
		printf("OnRadioRxDone: case FRAME_TYPE_JOIN_ACCEPT -> micRX=mic -> MlmeConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK\n");
...
//                LoRaMacParams.ChannelsDatarate = LoRaMacParamsDefaults.ChannelsDatarate;
		...
		printf("OnRadioRxDone: case FRAME_TYPE_JOIN_ACCEPT -> micRX != mic -> MlmeConfirm.Status = LORAMAC_EVENT_INFO_STATUS_JOIN_FAIL\n");
            }
            break;
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
		    ...
		    printf("OnRadioRxDone: case FRAME_TYPE_DATA_UNCONFIRMED_DOWN -> isMicOk=true -> McpsIndication.Status = LORAMAC_EVENT_INFO_STATUS_OK\n");
                    ...
                    McpsConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK;
		    printf("OnRadioRxDone: case FRAME_TYPE_DATA_UNCONFIRMED_DOWN -> isMicOk=true -> McpsConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK\n");
        ...
        case FRAME_TYPE_PROPRIETARY:
            {
                ...
                McpsIndication.Status = LORAMAC_EVENT_INFO_STATUS_OK;
		printf("OnRadioRxDone: case FRAME_TYPE_PROPRIERARY -> McpsIndication.Status = LORAMAC_EVENT_INFO_STATUS_OK\n");
                McpsIndication.Buffer = LoRaMacRxPayload;
...
static void OnRadioTxTimeout( void )
{
    printf("OnRadioTxTimeout func start...\n");
    if( LoRaMacDeviceClass != CLASS_C )
    {
        Radio.Sleep( );
    }
...
static void OnRadioRxError( void )
{
    printf("OnRadioRxError func start...\n");
    if( LoRaMacDeviceClass != CLASS_C )
    {
        Radio.Sleep( );
    }
...
static void OnRadioRxTimeout( void )
{
    printf("OnRadioRxTimeout func start...\n");
    if( LoRaMacDeviceClass != CLASS_C )
    {
        Radio.Sleep( );
    }
...
static void OnMacStateCheckTimerEvent( void )
{
    printf("OnMacStateCheckTimerEvent func start...\n");
    ...
    if( LoRaMacFlags.Bits.MacDone == 1 )
    {
        ...
        if( ( NodeAckRequested == false ) && ( txTimeout == false ) )
        {
		    ...
		    printf("OnMacStateCheckTimerEvent -> 1675line Procedure for the join request\n");
                    MlmeConfirm.NbRetries = JoinRequestTrials;

                    if( MlmeConfirm.Status == LORAMAC_EVENT_INFO_STATUS_OK )
                    {// Node joined successfully
			printf("OnMacStateCheckTimerEvent: if MlmeConfirm.Status == LORAMAC_EVENT_INFO_STATUS_OK(Node joined successfully)\n");
                        UpLinkCounter = 0;
                    ...
                    else
                    {
                        ...
                        {
			    printf("OnMacStateCheckTimerEvent -> else -> if -> Not Node joined successfully -> JoinRequestTrials >= MaxJoinRequestrials\n");
                            LoRaMacState &= ~LORAMAC_TX_RUNNING;
                        }
                        else
                        {
		            printf("OnMacStateCheckTimerEvent -> else -> else -> Not Node joined successfully -> Sends the same frame again\n");
...
static void OnTxDelayedTimerEvent( void )
{
    printf("OnTxDelayedTimerEvent func start...\n");
    LoRaMacHeader_t macHdr;
    LoRaMacFrameCtrl_t fCtrl;
	...
	printf("OnTxDelayedTimerEvent -> if end...\n");
    }
...
static void OnRxWindow2TimerEvent( void )
{
    printf("OnRxWindow2TimerEvent func start...\n");
...
static void OnAckTimeoutTimerEvent( void )
{
    printf("OnAckTimeoutTimerEvent func start...\n");
    TimerStop( &AckTimeoutTimer );
...
static bool SetNextChannel( TimerTime_t* time )
{ 
    printf("SetNextChannel func start...\n");
    ...
    if( AggregatedTimeOff <= TimerGetElapsedTime( AggregatedLastTxDoneTime ) )
    {
        ...
        for( uint8_t i = 0; i < LORA_MAX_NB_BANDS; i++ )
        {
            ...
            {
		printf("SetNextChannel -> IsLoRaMacNetworkJoined == false ) || ( DutyCycleOn == true)\n");
            ...
            for( uint8_t j = 0; j < 16; j++ )
            {
...
#endif
                {
                    ...
                    if( IsLoRaMacNetworkJoined == false )
                    {
                        ...
                        {
                            continue;
			    printf("SetNextChannel -> JOIN_CHANNELS & ( 1 << j ) ) == 0\n");
                        }
                    }
#endif
...
static bool RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    printf("RxWindowSetup func start...\n");
...
static bool Rx2FreqInRange( uint32_t freq )
{
    printf("Rx2FreqInRange  func start...\n");
#if defined( USE_BAND_433 ) || defined( USE_BAND_780 ) || defined( USE_BAND_868 )
...
static bool ValidatePayloadLength( uint8_t lenN, int8_t datarate, uint8_t fOptsLen )
{
    printf("ValidatePayloadLength func start...\n");
    uint16_t maxN = 0;
...
static uint8_t CountNbEnabled125kHzChannels( uint16_t *channelsMask )
{
    printf("CountNbEnabled125K~ func start...\n");
...
static void ReenableChannels( uint16_t mask, uint16_t* channelsMask )
{
    printf("ReenableChannels func start...\n");
...
static bool ValidateChannelMask( uint16_t* channelsMask )
{
    printf("ValidateChannelMask func start...\n");
...
static bool ValidateDatarate( int8_t datarate, uint16_t* channelsMask )
{
    printf("ValidateDatarate func start...\n");
...
static int8_t LimitTxPower( int8_t txPower, int8_t maxBandTxPower )
{
    printf("LimitTxPower func start...\n");
...
static bool ValueInRange( int8_t value, int8_t min, int8_t max )
{
    printf("ValueInRange func start...\n");
...
static bool DisableChannelInMask( uint8_t id, uint16_t* mask )
{
    printf("DisableChannelInMask func start...\n");
    uint8_t index = 0;
    index = id / 16;
...
static LoRaMacStatus_t AddMacCommand( uint8_t cmd, uint8_t p1, uint8_t p2 )
{
    printf("AddMacCommand func start...\n");
...
static uint8_t ParseMacCommandsToRepeat( uint8_t* cmdBufIn, uint8_t length, uint8_t* cmdBufOut )
{
    printf("ParseMacCommandsToRepeat func start...\n");
    uint8_t i = 0;
    uint8_t cmdCount = 0;
...
static void ProcessMacCommands( uint8_t *payload, uint8_t macIndex, uint8_t commandsSize, uint8_t snr )
{
    printf("ProcessMacCommands func start...\n");
    while( macIndex < commandsSize )
    {
        ...
        switch( payload[macIndex++] )
        {
            case SRV_MAC_LINK_CHECK_ANS:
                MlmeConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK;
		printf("ProcessMacCommands: case SRV_MAC_LINK_CHECK_ANS -> LORAMAC_EVENT_INFO_STATUS_OK\n");
...
static LoRaMacStatus_t ScheduleTx( void )
{
    ...
    if( IsLoRaMacNetworkJoined == false )
    {
	printf("ScheduleTx -> IsLoRaMacNetworkJoined == false\n");
...
static uint16_t JoinDutyCycle( void )
{
    printf("JoinDutyCycle func start...");
...
static void CalculateBackOff( uint8_t channel )
{
    printf("CalculateBackOff func start...\n");
	...
	printf("CalculateBackOff -> IsLoRaMacNetworkJoined == false\n");
    }
    else
    {
        if( DutyCycleOn == true )
...
static int8_t AlternateDatarate( uint16_t nbTrials )
{
    printf("AlternateDatarate func start...\n");
...
static void ResetMacParameters( void )
{
    printf("ResetMacParameters func start...\n");
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    printf("PrepareFrame func start...\n");
    uint16_t i;
    uint8_t pktHeaderLen = 0;
    uint32_t mic = 0;
    ...
    switch( macHdr->Bits.MType )
    {
	    ...
	    printf("PrepareFrame -> case FRAME_TYPE_JOIN_REQ\n");
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    printf("SendFrameOnChannel func start...\n");
    ...
    {
        JoinRequestTrials++;
	printf("SendFrameOnChannel -> IsLoRaMacNetworkJoined == false\n");
    }
...
LoRaMacStatus_t SetTxContinuousWave( uint16_t timeout )
{
    printf("SetTxContinuousWave func start...\n");
    int8_t txPowerIndex = 0;
    int8_t txPower = 0;
...
LoRaMacStatus_t SetTxContinuousWave1( uint16_t timeout, uint32_t frequency, uint8_t power )
{
    printf("SetTxContinuousWave1 func start...\n");
...
{
    printf("LoRaMacInitialization func start...\n");
    if( primitives == NULL )
    {
...
//���ǡ���1301/1308����оƬֻ��8�������ŵ���������Ҫ����SDK���ŵ����룬ֻ������Ҫ���ŵ�
  ...
  #error "please define USE_BAND_470 OR USE_BAND_470PREQUEL."
    ...
    Radio.Sleep( );
    printf("LoRaMacInitialization is end....\n");
    return LORAMAC_STATUS_OK;
}

LoRaMacStatus_t LoRaMacQueryTxPossible( uint8_t size, LoRaMacTxInfo_t* txInfo )
{
    printf("LoRaMacQueryTxPossible func start...\n");
...
{
    printf("LoRaMacMibGetRequestConfirm func start...\n");
    ...
    switch( mibGet->Type )
    {
	    ...
	    printf("LoRaMacMibGetRequestConfirm -> case MIB_NETWORK_JOINED\n");
            break;
        }
        case MIB_ADR:
        {
	    ...
	    printf("LoRaMacMibGetRequestConfirm -> case MIB_JOIN_ACCEPT_DELAY_1\n");
            break;
        }
        case MIB_JOIN_ACCEPT_DELAY_2:
        {
            mibGet->Param.JoinAcceptDelay2 = LoRaMacParams.JoinAcceptDelay2;
	    printf("LoRaMacMibGetRequestConfirm -> case MIB_JOIN_ACCEPT_DELAY_2\n");
            break;
        }
...
{
    printf("LoRaMacMibSetRequestConfirm func start...\n");
    LoRaMacStatus_t status = LORAMAC_STATUS_OK;
    ...
    switch( mibSet->Type )
    {
	    ...
	    printf("LoRaMacMibSetRequestConfirm -> case MIB_NETWORK_JOINED\n");
            break;
        }
        case MIB_ADR:
        {
	    ...
	    printf("LoRaMacMibSetRequestConfirm -> case MIB_JOIN_ACCEPT_DELAY_1\n");
            break;
        }
        case MIB_JOIN_ACCEPT_DELAY_2:
        {
            LoRaMacParams.JoinAcceptDelay2 = mibSet->Param.JoinAcceptDelay2;
	    printf("LoRaMacMibSetRequestConfirm -> case MIB_JOIN_ACCEPT_DELAY_2\n");
            break;
        }
        case MIB_CHANNELS_DEFAULT_DATARATE:
        {
...
{
    printf("LoRaMacChannelAdd func start...\n");
#if defined( USE_BAND_470 ) || defined( USE_BAND_470PREQUEL ) || defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID )
    return LORAMAC_STATUS_PARAMETER_INVALID;
#else
...
{
    printf("LoRaMacChannelRemove func start...\n");
#if defined( USE_BAND_433 ) || defined( USE_BAND_780 ) || defined( USE_BAND_868 )
    if( ( LoRaMacState & LORAMAC_TX_RUNNING ) == LORAMAC_TX_RUNNING )
    {
        if( ( LoRaMacState & LORAMAC_TX_CONFIG ) != LORAMAC_TX_CONFIG )
...
{
    printf("LoRaMacMulticastChannelLink func start...\n");
...
{
    printf("LoRaMacMulticastChannelUnlink func start...\n");
    if( channelParam == NULL )
    {
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
...
{
    printf("LoRaMacMlmeRequest func start...\n");
    ...
    switch( mlmeRequest->Type )
    {
	    ...
	    printf("LoRaMacMlmeRequest func end...\n");    
            break;
        }
...
{
    printf("LoRaMacMcpsRequest func start...\n");
    LoRaMacStatus_t status = LORAMAC_STATUS_SERVICE_UNKNOWN;
    LoRaMacHeader_t macHdr;
    uint8_t fPort = 0;
    void *fBuffer;
...
{
    printf("LoRaMacTestRxWindowsOn func start...\n");
    IsRxWindowsEnabled = enable;
}

void LoRaMacTestSetMic( uint16_t txPacketCounter )
{
    printf("LoRaMacTestSetMic func start...\n");
...
{
    printf("LoRaMacTestSetDutyCycleOn func start...\n");
#if ( defined( USE_BAND_868 ) || defined( USE_BAND_433 ) || defined( USE_BAND_780 ) )
    DutyCycleOn = enable;
#else
    DutyCycleOn = false;
...
{
    printf("LoRaMacTestSetChannel func start...\n");
    Channel = channel;
}

static RxConfigParams_t ComputeRxWindowParameters( int8_t datarate, uint32_t rxError )
{
    printf("ComputeRxWindowParameters func start...\n");
